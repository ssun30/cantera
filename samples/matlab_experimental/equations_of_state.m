...
%% Check if python is loaded for use of Cool Prop
%
...
if p.Status == "Loaded"
    ...
    if ispc
        pyenv('Version', [ctRoot, '/python.exe']);
    elseif ismac
        pyenv('Version', [ctRoot, '/bin/python']);
    elseif isunix
        pyenv('Version', [ctRoot, '/bin/python']);
    else
...
%
% The fuction "get_thermo_CoolProp" utilizes the CoolProp package to
% evaluate thermodymanic properties based on the thermodynamic state (T, p)
% for a given fluid. The HEOS for \mathrm{ CO_2 } used in this example is
% obtained from
% http://www.coolprop.org/fluid_properties/fluids/CarbonDioxide.html.
%
% Since the standard-reference thermodynamic states are different for
% Cantera and CoolProp, is is necessary to convert these values to an
% appropriate scale before comparsion. Therefore, both functions
% "get_thermo_Cantera" and "get_thermo_CoolProp" return the thermodynamic
...
function [h, u, s, cp, cv] = get_thermo_CoolProp(T, p)
    n = length(p);
    u = zeros(1, n);
    h = zeros(1, n);
    s = zeros(1, n);
    ...
    for i = 1:n
        u(i)  = py.CoolProp.CoolProp.PropsSI("U", "P", p(i), "T", T, "CO2");
        h(i)  = py.CoolProp.CoolProp.PropsSI("H", "P", p(i), "T", T, "CO2");
        s(i)  = py.CoolProp.CoolProp.PropsSI("S", "P", p(i), "T", T, "CO2");
        cp(i) = py.CoolProp.CoolProp.PropsSI("C", "P", p(i), "T", T, "CO2");
        cv(i) = py.CoolProp.CoolProp.PropsSI("O", "P", p(i), "T", T, "CO2");
    end
...
function plotEoS(p, ideal, rk, cp, y_label)
    figure;
    plot(p / 1e5, ideal, 'b-', 'LineWidth', 2); 
    hold on;
    plot(p / 1e5, rk, 'r-', 'LineWidth', 2);
    plot(p / 1e5, cp, 'w-', 'LineWidth', 2);
    xlabel('Pressure [bar]');
    ylabel(y_label);
    legend('Ideal EoS', 'R-K EoS', 'CoolProp');
    grid on;
end
...
% Plot the results
plotEoS(p, u_ideal, u_RK, u_CoolProp, "Relative Internal Energy [kJ/kg]");
plotEoS(p, h_ideal, h_RK, h_CoolProp, "Relative Enthalpy [kJ/kg]");
plotEoS(p, s_ideal, s_RK, s_CoolProp, "Relative Entropy [kJ/kg-K]");
...
plotEoS(p, cp_ideal, cp_RK, cp_CoolProp, "C_p [kJ/kg-K]");
% Specific heat at constant volume
plotEoS(p, cv_ideal, cv_RK, cv_CoolProp, "C_v [kJ/kg-K]");
%%
...
for i = 1:length(p_arr)
    for j = 1:length(T_arr)
        ...
        % CoolProp via Python (requires CoolProp installed in Python)
        density_CP(i, j) = py.CoolProp.CoolProp.PropsSI('D', 'P', P, 'T', T, 'CO2');
    end
end
...
for i = 1:length(p_arr)
    ...
    % CoolProp lines (solid)
    CP_line = plot(density_CP(i,:), T_arr, '-', 'Color', colors(i,:), 'HandleVisibility', 'on');
end
xlabel('Density [kg/m^3]');

ylabel('Temperature [K]');
legend([ideal_line(1), RK_line(1), CP_line(1)], {"Ideal EoS", "R-K EoS", "CoolProp"}, 'Location', 'northeast');
